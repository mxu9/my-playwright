# 银行明细对账单子代理 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 实现银行明细对账单处理子代理，从 PDF 中提取公司信息（户名、账号）和月度期末余额。

**Architecture:** 两步流程：OCR 扫描 → LLM 定位相关页面 → 多模态 LLM 提取结构化数据。参照 bank_confirmation_subagent.py 模式，添加可配置滑动窗口（默认关闭）。

**Tech Stack:** Python, PaddleOCR, pypdfium2, langchain_openai (LLMClient), base64 图片
...
| 文件 | 责任 |
|------|------|
| `audit_helper_poc/subagents/bank_detail_subagent.py` | 完整实现：两步流程、配置读取、错误处理 |
| `audit_helper_poc/.env.example` | 添加 BANK_DETAIL_WINDOW_SIZE 配置说明 |

---

### Task 1: 实现银行明细对账单子代理完整代码

**Files:**
- Modify: `audit_helper_poc/subagents/bank_detail_subagent.py` (完整重写)

**Context:** 参考 `bank_confirmation_subagent.py` 的两步流程模式，使用 `pdf_utils.py` 的 OCR 和图片转换工具，使用 `llm_utils.py` 的 LLMClient。

- [ ] **Step 1: 完整重写 bank_detail_subagent.py**
...
Run: `python -m py_compile audit_helper_poc/subagents/bank_detail_subagent.py`
Expected: 无输出（语法正确）

- [ ] **Step 3: Commit bank_detail_subagent.py**

```bash
git add audit_helper_poc/subagents/bank_detail_subagent.py
git commit -m "feat(bank_detail): 实现银行明细对账单子代理两步流程"
```

---

### Task 2: 更新 .env.example 添加配置说明

**Files:**
- Modify: `audit_helper_poc/.env.example`

- [ ] **Step 1: 添加 BANK_DETAIL_WINDOW_SIZE 配置说明**

在 `BANK_CONFIRMATION_WINDOW_SIZE=0` 之后添加：

```diff
# -------------------
# Bank Confirmation Parameters
# -------------------
# 银行询证函滑动窗口大小（默认 0 = 关闭）
# 设置为 1 或更大可在定位页面前后扩展相邻页
BANK_CONFIRMATION_WINDOW_SIZE=0

# -------------------
# Bank Detail Parameters
# -------------------
# 银行明细对账单滑动窗口大小（默认 0 = 关闭）
# 设置为 1 或更大可在定位页面前后扩展相邻页
BANK_DETAIL_WINDOW_SIZE=0
```
...
git commit -m "docs: 添加 BANK_DETAIL_WINDOW_SIZE 配置说明"
```
...
**1. Spec coverage:**
- 输出数据结构（company_info + monthly_balances）✓ Task 1 实现
- 滑动窗口可配置（默认关闭）✓ Task 1 实现，Task 2 配置
- 两步流程 ✓ Task 1 实现
- Prompt 设计 ✓ Task 1 实现
- 错误处理 ✓ Task 1 实现
...
**3. Type consistency:**
- `invoke` 返回 dict ✓
- `_locate_pages` 返回 list[int] ✓
- `_extract_info` 返回 dict ✓
- 字段名与设计文档一致 ✓